...
public class Logu {
    private static final Logger logger = Logger.getLogger(Logu.class.getName());
    private static FileHandler fileHandler;

    static {
        try {
            fileHandler = new FileHandler("Logu.log");
            logger.addHandler(fileHandler);
            SimpleFormatter formatter = new SimpleFormatter();
            fileHandler.setFormatter(formatter);
            logger.setLevel(Level.ALL);
        } catch (Exception e) {
            logger.log(Level.SEVERE, "Ошибка настройки fileHandler", e);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            EmployeeManagers manager = new EmployeeManagers();
            boolean exit = false;
            while (!exit) {
                try {
                    ...
                    switch (choice) {
                        ...
                        case 8:
                            ...
                            String nameToBuryDirector = scanner.nextLine();
                            employees = manager.buryDirector(employees, nameToBuryDirector);
                            break;
                        case 9:
                    ...
                    logger.log(Level.SEVERE, "Ошибка в меню", e);
                }
            }
            System.out.println("Программа завершена.");
        } catch (Exception e) {
            logger.log(Level.SEVERE, "Обнаружена общая ошибка", e);
        }
    }

    private static void printEmployees(String[][] employees) {
    ...
    private static String[][] addEmployee(String[][] employees, String name, String role) {
    ...
    private static String[][] changeEmployeeName(String[][] employees, int index, String newName) {
        if (index < 0 || index >= employees.length) {
            System.out.println("Некорректный индекс сотрудника.");
            return employees;
        }
